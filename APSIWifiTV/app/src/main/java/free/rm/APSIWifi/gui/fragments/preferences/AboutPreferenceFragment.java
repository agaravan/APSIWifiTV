...
public class AboutPreferenceFragment extends PreferenceFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		websitePref.setSummary(BuildConfig.APSI_WEBSITE);
		websitePref.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
			@Override
			public boolean onPreferenceClick(Preference preference) {
				// view the app's website in a web browser
				Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(BuildConfig.APSI_WEBSITE));
	...
	private static String getAppBuildTimeStamp() {
		...
		try {
			ApplicationInfo appInfo = APSIWifiApp.getContext().getPackageManager().getApplicationInfo(APSIWifiApp.getContext().getPackageName(), 0);
	...
	private void displayCredits() {
		...
		webView.loadUrl(BuildConfig.APSI_WEBSITE_CREDITS);
