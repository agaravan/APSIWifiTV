...
import free.rm.APSIWifi.gui.activities.MainActivity;
import free.rm.APSIWifi.gui.activities.ThumbnailViewerActivity;
import free.rm.APSIWifi.gui.businessobjects.PlayerViewGestureDetector;
import free.rm.APSIWifi.gui.businessobjects.APSIWifiMaterialDialog;
import free.rm.APSIWifi.gui.businessobjects.adapters.CommentsAdapter;
import free.rm.APSIWifi.gui.businessobjects.fragments.ImmersiveModeFragment;
import free.rm.APSIWifi.gui.businessobjects.views.ClickableLinksTextView;
import free.rm.APSIWifi.gui.businessobjects.views.SubscribeButton;
import free.rm.APSIWifi.R;
import free.rm.APSIWifi.app.APSIWifiApp;
import free.rm.APSIWifi.businessobjects.YouTube.GetVideosDetailsByIDs;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeChannel;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeChannelInterface;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeVideo;
import free.rm.APSIWifi.businessobjects.YouTube.Tasks.GetVideoDescriptionTask;
import free.rm.APSIWifi.businessobjects.YouTube.Tasks.GetYouTubeChannelInfoTask;
import free.rm.APSIWifi.businessobjects.YouTube.VideoStream.StreamMetaData;
import free.rm.APSIWifi.businessobjects.db.DownloadedVideosDb;
import free.rm.APSIWifi.businessobjects.db.PlaybackStatusDb;
import free.rm.APSIWifi.businessobjects.db.Tasks.CheckIfUserSubbedToChannelTask;
import free.rm.APSIWifi.businessobjects.db.Tasks.IsVideoBookmarkedTask;
import free.rm.APSIWifi.businessobjects.interfaces.GetDesiredStreamListener;
import free.rm.APSIWifi.businessobjects.interfaces.YouTubePlayerFragmentInterface;
...
//		});
		...
		if (youTubeVideo == null) {
			...
			if (bundle != null  &&  bundle.getSerializable(YOUTUBE_VIDEO_OBJ) != null) {
				...
				// ... or the video URL is passed to APSI Wifi via another Android app
	...
	private void setUpHUDAndPlayVideo() {
		...
		if(!APSIWifiApp.getPreferenceManager().getBoolean(getString(R.string.pref_key_disable_playback_status), false) && PlaybackStatusDb.getVideoDownloadsDb().getVideoWatchedStatus(youTubeVideo).position > 0) {
			new APSIWifiMaterialDialog(getContext())
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		if((youTubeVideo != null && !youTubeVideo.isDownloaded()) && (APSIWifiApp.isConnectedToWiFi() || (APSIWifiApp.isConnectedToMobile() && allowDownloadsOnMobile))) {
	...
	public void videoPlaybackStopped() {
		...
		if(!APSIWifiApp.getPreferenceManager().getBoolean(getString(R.string.pref_key_disable_playback_status), false)) {
	...
	private class GetVideoDetailsTask extends AsyncTaskParallel<Void, Void, YouTubeVideo> {
		...
		/**
		 * The video URL is passed to APSI Wifi via another Android app (i.e. via an intent).
		 *
	...
	private static class VideoBrightness {
		...
		private void saveBrightnessToPreference() {
			SharedPreferences.Editor editor = APSIWifiApp.getPreferenceManager().edit();
		...
		private void loadBrightnessFromPreference() {
			final float brightnessPref = APSIWifiApp.getPreferenceManager().getFloat(SAVE_BRIGHTNESS_FLAG, 1);
