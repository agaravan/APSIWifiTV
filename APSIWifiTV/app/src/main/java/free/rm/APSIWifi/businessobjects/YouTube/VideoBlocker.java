...
package free.rm.APSIWifi.businessobjects.YouTube;
...
import free.rm.APSIWifi.app.APSIWifiApp;import free.rm.APSIWifi.businessobjects.Logger;import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeChannel;import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeVideo;import free.rm.APSIWifi.R;
import free.rm.APSIWifi.app.APSIWifiApp;
import free.rm.APSIWifi.businessobjects.Logger;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeChannel;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeVideo;
import free.rm.APSIWifi.businessobjects.db.ChannelFilteringDb;
import free.rm.APSIWifi.app.APSIWifiApp;
import free.rm.APSIWifi.businessobjects.Logger;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeChannel;
import free.rm.APSIWifi.businessobjects.YouTube.POJOs.YouTubeVideo;

import static free.rm.APSIWifi.app.APSIWifiApp.getStr;
...
public class VideoBlocker {
	...
	private static final Set<String> defaultPrefLanguages = new HashSet<>(APSIWifiApp.getStringArrayAsList(R.array.languages_iso639_codes));
	...
	public List<YouTubeVideo> filter(List<YouTubeVideo> videosList) {
		...
		final Set<String>   preferredLanguages    = APSIWifiApp.getPreferenceManager().getStringSet(APSIWifiApp.getStr(R.string.pref_key_preferred_languages), defaultPrefLanguages);
	...
	public static boolean isChannelBlacklistEnabled() {
		final String channelFilter = APSIWifiApp.getPreferenceManager().getString(APSIWifiApp.getStr(R.string.pref_key_channel_filter_method), APSIWifiApp.getStr(R.string.channel_blacklisting_filtering));
		return channelFilter.equals(APSIWifiApp.getStr(R.string.channel_blacklisting_filtering));
	}
	...
	private boolean filterByLanguageDetection(YouTubeVideo video, Set<String> preferredLanguages) {
		...
		if (!APSIWifiApp.getPreferenceManager().getBoolean(APSIWifiApp.getStr(R.string.pref_key_lang_detection_video_filtering), false))
			return false;
	...
	private BigInteger getViewsFilteringValue() {
		final  String viewsFiltering = APSIWifiApp.getPreferenceManager().getString(APSIWifiApp.getStr(R.string.pref_key_low_views_filter), APSIWifiApp.getStr(R.string.views_filtering_disabled));
	...
	private boolean filterByViews(YouTubeVideo video, BigInteger minimumVideoViews) {
		...
		if (video.getViewsCountInt().compareTo(minimumVideoViews) < 0) {
			log(video, FilterType.VIEWS, String.format(APSIWifiApp.getStr(R.string.views), video.getViewsCountInt()));
			return true;
		}
	...
	private int getDislikesFilteringValue() {
		final  String dislikesFiltering = APSIWifiApp.getPreferenceManager().getString(APSIWifiApp.getStr(R.string.pref_key_dislikes_filter), APSIWifiApp.getStr(R.string.dislikes_filtering_disabled));
	...
	private boolean filterByDislikes(YouTubeVideo video, int minimumVideoDislikes) {
		...
		if (dislikesPercentage >= minimumVideoDislikes) {
			log(video, FilterType.DISLIKES, String.format(APSIWifiApp.getStr(R.string.dislikes), dislikesPercentage));
			return true;
		}
