package free.rm.APSIWifi.gui.fragments.preferences;
...
import free.rm.APSIWifi.gui.businessobjects.MultiSelectListPreferenceDialog;
import free.rm.APSIWifi.gui.businessobjects.MultiSelectListPreferenceItem;
...
import free.rm.APSIWifi.BuildConfig;
...
import free.rm.APSIWifi.businessobjects.YouTube.VideoStream.HttpDownloader;
...
public class VideoBlockerPreferenceFragment extends PreferenceFragment {
	...
	private void initChannelFilteringPreferences(Preference channelBlacklistPreference, Preference channelWhitelistPreference) {
		...
		final String channelFilter = APSIWifiApp.getPreferenceManager().getString(getString(R.string.pref_key_channel_filter_method), getString(R.string.channel_blacklisting_filtering));
	...
	private class PreferredLanguageDialog extends MultiSelectListPreferenceDialog {
			...
			onPositive(new MaterialDialog.SingleButtonCallback() {
				...
				public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
					...
					if (preferredLangIsoCodes.size() > 0) {
						SharedPreferences sharedPref = APSIWifiApp.getPreferenceManager();
		...
		private Set<String> getPreferredLanguages() {
			SharedPreferences pref = APSIWifiApp.getPreferenceManager();
	...
	private class GetChannelIdFromUrlTask extends AsyncTaskParallel<Void, Void, MultiSelectListPreferenceItem> {
		...
		protected void onPreExecute() {
			getChannelInfoDialog = new APSIWifiMaterialDialog(getActivity())
					.progress(true, 0)
