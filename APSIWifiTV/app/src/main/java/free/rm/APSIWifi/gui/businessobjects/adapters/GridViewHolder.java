...
import free.rm.APSIWifi.businessobjects.db.Tasks.IsVideoWatchedTask;
...
class GridViewHolder extends RecyclerView.ViewHolder {
	...
	public void updateViewsData(Context context) {
		...
		if(APSIWifiApp.getPreferenceManager().getBoolean(context.getString(R.string.pref_key_disable_playback_status), false)) {
 	...
 	private void onOptionsButtonClick(final View view) {
		...
		if(youTubeVideo.isDownloaded()) {
			...
			if(APSIWifiApp.isConnectedToWiFi() || (APSIWifiApp.isConnectedToMobile() && allowDownloadsOnMobile))
