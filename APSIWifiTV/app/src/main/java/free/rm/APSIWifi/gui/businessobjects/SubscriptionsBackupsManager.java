...
import free.rm.APSIWifi.gui.businessobjects.preferences.BackupDatabases;
...
import free.rm.APSIWifi.businessobjects.db.Tasks.UnsubscribeFromAllChannelsTask;
...
public class SubscriptionsBackupsManager {
	...
	private void displayFilePicker(final boolean importDb) {
		...
		properties.extensions     = importDb ? new String[]{"apsiwifi"} : new String[]{"xml", "subscription_manager"};
	...
	public void displayImportSubscriptionsFromYouTubeDialog() {
		...
		new APSIWifiMaterialDialog(activity)
				.title(R.string.import_subscriptions)
				.content(msg)
	...
	private class SubscribeToImportedChannelsTask extends AsyncTaskParallel<List<MultiSelectListPreferenceItem>, Void, Integer> {
		...
		protected void onPostExecute(Integer totalChannelsSubscribedTo) {
			...
			Toast.makeText(activity,
							String.format(APSIWifiApp.getStr(R.string.subscriptions_to_channels_imported), totalChannelsSubscribedTo),
